...
public:
    dst_fr_t m_fr; // Contains frame based header information
    ft_t m_ft; // Contains FIR-coef. compression data
    pt_t m_pt; // Contains Ptable-entry compression data
    vector<array<unsigned int, DST_AC_HISMAX> > P_one; // Probability table for arithmetic coder
    vector<uint8_t> AData;
    // Contains the arithmetic coded bit stream of a complete frame	vector<uint8_t> AData;               // Contains the arithmetic coded bit stream of a complete frame
...
private:
    ...
    void fillTable4Bit(dst_segment_t &S, vector<vector<uint8_t> > &Table4Bit);
